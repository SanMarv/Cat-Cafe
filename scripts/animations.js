...
function fadeMenu(day){
    let items = document.getElementsByClassName("item")
    $(".item").fadeOut("slow", function(){
        for(let i = 0; i < items.length; i++){
            items[i].textContent = menus[day][i]
        }
    })
...
function showSlides(n){
    ...
    $("#c-name").fadeOut("slow", function(){
        ...
        element.textContent = cats[slideIndex - 1]["name"]
    })
    $("#c-name").fadeIn("slow")
    $("#c-desc").fadeOut("slow", function(){
        element = document.getElementById("c-desc")
        element.textContent = cats[slideIndex - 1]["desc"]
    })
    $("#c-desc").fadeIn("slow")
...
function changeImg(){
    ...
    items.forEach(item =>{
        item.addEventListener("mouseenter",() =>{
            ...
            document.querySelector(".menu-img").src = item_imgs[text]
